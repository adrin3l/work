 ...
 */
?>
<?php GLOBAL $swift_options,$swift_design_options; ?>
<?php
...
?>
	<article id="post-<?php the_ID(); ?>" <?php post_class('clearfix blog-style'); ?>>
		...
		<?php if ( (!$full_length) && (is_search() || isset( $swift_options['excerpts_enable'] ) && $swift_options['excerpts_enable']) ) : // Only display Excerpts for Search ?>
		...
		<?php 	if ( 'post' == get_post_type() ) : // Hide category and tag text for pages on Search
			...
			<div class="comments-link"><?php comments_popup_link( '<span class="leave-reply">'._x( '0', '0 comments', 'swift' ).'</span>', _x( '1', '1 comment', 'swift' ), sprintf( _x( '%s', 'n comments', 'swift' ), '%' ) ); ?></div>
