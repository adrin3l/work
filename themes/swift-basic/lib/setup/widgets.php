...
?>
		<h4><?php _e('Set your affiliate id at Swift Options -> Ad Management. If you dont have an affiliate id signup','swift')?> <a href="http://swiftthemes.com/swiftlers-area/aff/signup"><?php _e('here','swift') ?></a></h4>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'swift'); ?></label>
...
class Swift_Widget_Text_No_Styling extends WP_Widget {
	...
	function form( $instance ) {
		...
		$text = esc_textarea($instance['text']);
?>
		<h4><?php printf( __( '(This widget will be removed after June 1st, functionality of this widget is included in the %sSmart text widget%s.)', 'swift' ), '<strong>', '</strong>'); ?></h4>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'swift'); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<textarea class="widefat" rows="16" cols="20" id="<?php echo $this->get_field_id('text'); ?>" name="<?php echo $this->get_field_name('text'); ?>"><?php echo $text; ?></textarea>

		<p><input id="<?php echo $this->get_field_id('filter'); ?>" name="<?php echo $this->get_field_name('filter'); ?>" type="checkbox" <?php checked(isset($instance['filter']) ? $instance['filter'] : 0); ?> />&nbsp;<label for="<?php echo $this->get_field_id('filter'); ?>"><?php _e('Automatically add paragraphs', 'swift'); ?></label></p>
<?php
	}
}
...
class Swift_Widget_Smart_Text extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
		if ( !empty( $title ) ) { echo $before_title . $title . $after_title; } ?>
			<div class="textwidget <?php echo $instance['nopadding'] ? 'nopadding' : ''; ?>"><?php echo $instance['filter'] ? wpautop($text) : $text; ?></div>
		<?php
		echo $after_widget;
	}
...
?>
		...
		<p><input id="<?php echo $this->get_field_id('filter'); ?>" name="<?php echo $this->get_field_name('filter'); ?>" type="checkbox" <?php checked(isset($instance['filter']) ? $instance['filter'] : 0); ?> />&nbsp;<label for="<?php echo $this->get_field_id('filter'); ?>"><?php _e('Automatically add paragraphs', 'swift'); ?></label></p>
		<p><input id="<?php echo $this->get_field_id('nopadding'); ?>" name="<?php echo $this->get_field_name('nopadding'); ?>" type="checkbox" <?php checked(isset($instance['nopadding']) ? $instance['nopadding'] : 0); ?> />&nbsp;<label for="<?php echo $this->get_field_id('nopadding'); ?>"><?php _e('Remove padding and borders', 'swift'); ?></label></p>
        <p><input id="<?php echo $this->get_field_id('hide_on_mobile'); ?>" name="<?php echo $this->get_field_name('hide_on_mobile'); ?>" type="checkbox" <?php checked(isset($instance['hide_on_mobile']) ? $instance['hide_on_mobile'] : 0); ?> />&nbsp;<label for="<?php echo $this->get_field_id('hide_on_mobile'); ?>"><?php _e('Hide this widget when viewed on mobile', 'swift'); ?></label></p>
...
class Swift_Widget_Text_Page_Specific extends WP_Widget {
	...
	function form( $instance ) {
		...
		$pages = get_pages();
		?>
		<h4><?php _e( 'Select the pages on which you want this widget to appear', 'swift' ); ?></h4>
		<style>
		ul.swift_multiselect li{
		list-style:none;float:left;width:50%}
		ul.swift_multiselect .checkbox{padding:5px;margin-right:10px}
		</style><ul class="swift_multiselect"><?php 
...
class Swift_Widget_Text_Category_Specific extends WP_Widget {
	...
	function form( $instance ) {
		...
		list-style:none;float:left;width:33%}
		ul.swift_multiselect_cat .checkbox{padding:5px;margin-right:10px}
		</style><ul class="swift_multiselect_cat"><?php 
...
?>
		...
		<li class="clearfix">
			...
			<span class="meta">
			<?php if($show_date):?>
			<?php _e('on','swift')?> <a href="<?php echo esc_url( get_permalink() )?>" title="<?php echo esc_attr( get_the_time() )?>" rel="bookmark"><time class="entry-date" datetime="<?php echo esc_attr( get_the_date( 'c' ) )?>" ><?php echo esc_html( get_the_date() )?></time></a>
			<?php endif?>
			
			<?php if($show_author_name): ?>
			<?php _e('by','swift')?> <a href="<?php echo esc_url(get_author_posts_url(get_the_author_meta('ID'))) ?>"><?php echo esc_attr(get_the_author()) ?></a>
			<?php endif;?>
			</span>
			<div class="clear"></div>
		</li>
...
<?php
	...
	function form( $instance ) {
		...
		?>
		<p><label for="<?php echo $this->get_field_id('title', 'swift' ); ?>"><?php _e('Title:', 'swift' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title', 'swift' ); ?>" name="<?php echo $this->get_field_name('title', 'swift' ); ?>" type="text" value="<?php echo $title; ?>" /></p>

		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of posts to show:', 'swift' ); ?></label>
		<input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
		
		<p><label for="<?php echo $this->get_field_id('age'); ?>"><?php _e('Show posts from last "n" days:', 'swift' ); ?></label>
		<input id="<?php echo $this->get_field_id('age'); ?>" name="<?php echo $this->get_field_name('age'); ?>" type="text" value="<?php echo $age; ?>" size="3" /></p>
		<p><i><label for="<?php echo $this->get_field_id('age'); ?>"><?php _e('Enter the value of "n" above', 'swift' ); ?></label></i></p>
		<br />
		<input class="checkbox" type="checkbox" <?php checked($images, true) ?> id="<?php echo $this->get_field_id('images'); ?>" name="<?php echo $this->get_field_name('images'); ?>" />
		<label for="<?php echo $this->get_field_id('images'); ?>"><?php _e('Show thumbnail', 'swift' ); ?></label><br />
		...
		<label for="<?php echo $this->get_field_id('show_date'); ?>"><?php _e('Show date', 'swift' ); ?></label><br />
		
		<input class="checkbox" type="checkbox" <?php checked($show_author_name, true) ?> id="<?php echo $this->get_field_id('show_author_name'); ?>" name="<?php echo $this->get_field_name('show_author_name'); ?>" />
		<label for="<?php echo $this->get_field_id('show_author_name'); ?>"><?php _e('Show author name', 'swift' ); ?></label><br />
		<?php
	}
}
/**
...
?>
		...
		<input class="checkbox" type="checkbox" <?php checked($show_button, true) ?> id="<?php echo $this->get_field_id('show_button'); ?>" name="<?php echo $this->get_field_name('show_button'); ?>" />
		<label for="<?php echo $this->get_field_id('show_button'); ?>"><?php _e('Show random post button in widget title', 'swift' ); ?></label><br />
		<br>
		<p><label for="<?php echo $this->get_field_id('button_text', 'swift' ); ?>"><?php _e('Random post button text:', 'swift' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('button_text', 'swift' ); ?>" name="<?php echo $this->get_field_name('button_text', 'swift' ); ?>" type="text" value="<?php echo $button_text; ?>" /></p>
		<?php
	}
}
/**
...
class Swift_Subscribe_Box extends WP_Widget {
    ...
    function widget($args, $instance) {		
        ...
        ?>
			...
			<h3 class="alignleft"><?php _e( 'Subscribe', 'swift' ); ?></h3>
			<a href="http://feeds.feedburner.com/<?php echo $feedId ?>" title="<?php _e( 'Subscribe to our RSS feed', 'swift' ); ?>">
			<img src="http://feeds.feedburner.com/~fc/<?php echo $feedId ?>?bg=<?php echo $swift_design_options['chicklet_bg'];?>&amp;fg=<?php echo $swift_design_options['chicklet'];?>&amp;anim=0" class="chicklet alignright" alt="<?php _e( 'Feedburner counter', 'swift' ); ?>"/></a>
			<div class="clear"></div>
    <form action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $feedId ?>', 'popupwindow', 'scrollbars=yes, width=550,height=520 ');return true">
            <input name="email" value="<?php _e( 'Enter your e-mail address', 'swift' ); ?>" onfocus="if (this.value == '<?php _e( 'Enter your e-mail address', 'swift' ); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e( 'Enter your e-mail address', 'swift' ); ?>';}" type="text" class="alignleft" />
            <input value="<?php echo $feedId ?>" name="uri" type="hidden" />
            <input name="loc" value="en_US" type="hidden" />
            <input value="<?php _e( 'Subscribe', 'swift' ); ?>" type="submit" class="alignright"/>
            </form>
        	<div class="clear"></div>
			</div>
			...
			<div class="g-plusone" data-size="medium" data-href="<?php echo $swift_options['sm_gplus'];?>"></div>
			...
			<li class="fb-like">
			<iframe src="//www.facebook.com/plugins/like.php?href=https%3A%2F%2Fwww.facebook.com%2F<?php echo $swift_options['sm_fb_page_id'];?>&amp;send=false&amp;layout=button_count&amp;width=60&amp;show_faces=false&amp;action=like&amp;colorscheme=light&amp;font&amp;height=21&amp;appId=140172592746809" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:100px; height:21px;" allowTransparency="true"></iframe>
			</li>
			
			<li class="twitter-follow last">
			<a href="https://twitter.com/<?php echo $swift_options['sm_twitter'];?>" class="twitter-follow-button" data-show-count="true" data-show-screen-name="false"><?php printf( __( 'Follow %s', 'swift' ), '@'.$swift_options['sm_twitter'] );?></a>
...
class Swift_Tabs extends WP_Widget {
    ...
    function widget($args, $instance) {		
     ...
     ?>
        <?php echo $before_widget; ?>
		<div id="tabs-widget" class="shortcode-tabs clearfix default">
			<ul class="tab_titles">
			<li class="nav-tab"><a href="#tab-2"><?php _e('Comments','swift')?></a></li>
			<li class="nav-tab"><a href="#tab-1"><?php _e('Recent posts','swift')?></a></li>
