...
function swift_generate_stylesheet(){
	...
	if( $d_o['blog_or_mag'] == 'magazine-full' && $d_o['layout'] != 'fluid'){
		$css .= 'body.mag-full #content{width:'.($temp_content).'px}';
	}
	...
	if(isset($d_o['full_slider_height']) && $d_o['full_slider_height'] != '' )
		$dimensions['full_width_slider'] = array($content-38, $d_o['full_slider_height'] );//20+2*8+2*1
	else
...
//		$css .= 'article.mag1{width:'.((100- 2*$columns) / $columns).'%;padding:0;overflow:hidden}';
	...
	if( $d_o['sb_position'] != 'right' ){
		$css .= 'li.comment .avatar{padding:5px!important; background:#EEE;float:left;margin:0 10px 0 0;width:32px;height:32px;}';
	}
	...
	*/
	...
	$css .= "#nav-single a{background:#{$d_o['post_nav_a_bg']};color:#{$d_o['post_nav_a']};border:none}";
	$css .= "#nav-single a:hover{background:#{$d_o['post_nav_a_hover_bg']};}";
...
border-bottom-right-radius:0;}';
	...
	$css .= '#below-logo-container{background:#'.$d_o['mo_nav_bg'].'!important}';
	$css .= '#below-logo-container a{background:#'.$d_o['mo_nav_a_bg'].'!important;color:#'.$d_o['mo_nav_a'].';border:none}';
	$css .= '}';
	endif;
