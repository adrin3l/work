...
<?php while ($recentPosts->have_posts()) : $recentPosts->the_post();?>
      	<li>
<?php 		the_post_thumbnail( $swift_thumbnail_sizes['full_width_slider'], array( 'class'	=> "slide-thumbnail"
											 )
						  );
			$swift_slider_posts_ids[] = get_the_ID();
?>        <div class="flex-caption">
			<h2 class="post-title"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'swift' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
			<p><?php the_excerpt()?></p>
		  </div>
      </li>
		<?php endwhile; ?>
