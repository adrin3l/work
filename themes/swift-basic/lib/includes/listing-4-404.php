        ...
        <div class="div-content">
        	...
        	<fieldset>
				<select name=\"archive-dropdown\" onChange='document.location.href=this.options[this.selectedIndex].value;'>
 					 <option value=\"\"><?php echo esc_attr(__('Select Month','swift')); ?></option>
		...
		<li class="clearfix">
					<?php  the_post_thumbnail( array(36,36), array('class' => 'alignleft thumb' ));?>

			<a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>"><?php if ( get_the_title() ) the_title(); else the_ID(); ?></a>
			<p class="meta date"><a href="<?php echo esc_url( get_permalink() )?>" title="<?php echo esc_attr( get_the_time() )?>" rel="bookmark"><time class="entry-date" datetime="<?php echo esc_attr( get_the_date( 'c' ) )?>" pubdate="pubdate"><?php echo esc_html( get_the_date() )?></time></a></p>
		</li>
		<?php endwhile; ?>
		</ol>
<?php 	endif;?>
...
<?php wp_reset_query(); ?>
		...
		<br />
	    <big><em><?php _e('Tag cloud','swift')?></em></big><br /><br />
		<div class="widget">
