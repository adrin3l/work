...
function localbusiness_optionsframework_add_admin() {
    global $query_string;

    $themename = localbusiness_get_option('of_themename');
    $shortname = localbusiness_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = localbusiness_get_option('of_template');
            localbusiness_reset_options($options, 'optionsframework');
            header("Location: admin.php?page=optionsframework&reset=true");
            die;
        }
    }
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'localbusiness_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'localbusiness_load_only');
}

add_action('admin_menu', 'localbusiness_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function localbusiness_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    if ($page == 'optionsframework') {
        localbusiness_delete_option('of_options');
    }
}
...
function localbusiness_optionsframework_options_page() {
    $options = localbusiness_get_option('of_template');
    $themename = localbusiness_get_option('of_themename');
    ?>
    <div class="trail-notify">
        <h1><?php _e('Get Local Business Theme PRO!','local-business'); ?></h1>
        <p><?php _e('You are Using the Lite Version of Local Business Theme. Upgrade to Pro for extra features like Inbuilt Lead Capture System at Home Page, Home Page Slider, Contact Page Template, Gallery Page Template, FullWidth Page Template, Multiple Color Options and much more.</p>
        <a href="http://www.inkthemes.com/wp-themes/local-business-wordpress-theme/" target="new">Upgrade to Local Business Theme PRO Now >>','local-business'); ?></a>
    </div>
    ...
    $return = localbusiness_optionsframework_machine($options);
    ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
...
function localbusiness_load_only() {
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = localbusiness_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = localbusiness_get_option($option_id);
                }
                ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - localbusiness_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'localbusiness_ajax_callback');

function localbusiness_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        localbusiness_update_option($clickedID, $uploaded_file['url']);
        ...
        localbusiness_delete_option($id);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = localbusiness_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            localbusiness_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    localbusiness_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    localbusiness_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            localbusiness_update_option($multicheck_id, 'false');
                        } else {
                            localbusiness_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    localbusiness_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    localbusiness_update_option($id, $border_array);
...
function localbusiness_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = localbusiness_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = localbusiness_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= localbusiness_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= localbusiness_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = localbusiness_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = localbusiness_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = localbusiness_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = localbusiness_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - localbusiness_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */

function localbusiness_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = localbusiness_get_option($id);

    if ($mod != 'min') {
        $val = $std;
        if (localbusiness_get_option($id) != "") {
            $val = localbusiness_get_option($id);
        }
